...
if __name__=='__main__':
    ...
    model=EdgeViT(config,n_classes=args.num_classes,img_size=args.img_size,fusion_type=args.fusion_type,
                              embed_dim=256,depth=8,num_heads=8).cuda()
    ckpt=torch.load(args.model_weight,map_location='cuda',weights_only=False)
    model.load_state_dict(ckpt['model_state_dict'] if 'model_state_dict' in ckpt else ckpt)
    print("✓ Model loaded\n")

    log_dir='./test_log/test_log_edgeguidedvit_v2'; os.makedirs(log_dir,exist_ok=True)
