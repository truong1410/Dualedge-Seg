"""Test script for DualEdge_UNet on Synapse — Dice, HD95, ED, ASD, NSD."""
...
def test_volume(image, label, model, num_classes, img_size, z_spacing=1, save_path=None, case=None):
    ...
    for si in range(D):
        ...
        with torch.no_grad():
            out, _ = model(x)
            p=torch.argmax(torch.softmax(out,1),1).squeeze(0).cpu().numpy()
...
def inference(args, model, save_path=None):
    ...
    model.eval()
    all_m={k:[] for k in ['dice','hd95','ed','asd','nsd']}
    case_m={}
    ...
    for i,cname in enumerate(CLASS_NAMES):
        logging.info(f"Class {i+1} ({cname:12s}): "
            f"Dice {np.mean(arr['dice'][:,i]):.4f}, "
            f"HD95 {np.nanmean(arr['hd95'][:,i]):.2f}mm, "
            f"ED {np.mean(arr['ed'][:,i]):.2f}, "
            f"ASD {np.nanmean(arr['asd'][:,i]):.2f}mm, "
            f"NSD {np.mean(arr['nsd'][:,i]):.4f}")
...
if __name__=='__main__':
    ...
    parser.add_argument('--output_dir',   default='./predictions_dualedge_swin')
    ...
    print(f"Loading DualEdge_Swin [{args.fusion_type}] from {args.model_weight}")
    model=DualEdge_Swin(config,n_classes=args.num_classes,
    ...
    log_dir='./test_log/test_log_dualedge_swin'; os.makedirs(log_dir,exist_ok=True)
